...
private:
    ...
    void device_poll() {
        ...
        image_transport::Publisher image_pub = it.advertise(image_topic_, 1);

        ros::Publisher cam_time_stamp_pub = nh.advertise<sensor_msgs::TimeReference>(time_topic_,1);
        ros::Publisher cam_info_pub = nh.advertise<sensor_msgs::CameraInfo>(info_topic_, 1);
        ...
        info_manager.loadCameraInfo(info_xml_);
        ...
        while (ros::ok())
        {
            ...
            if (!ocam->getImages(image, time_stamp))
            {
                usleep(1000);
                continue;
            } else {
            ...
            if (image_pub.getNumSubscribers() > 0) {
                publishImage(image, image_pub, frame_id_, now);
            }
            if (cam_info_pub.getNumSubscribers() > 0) {
                publishCamInfo(cam_info_pub, cam_info, now);
            }
...
public:
    ...
    oCamROS() {
        ...
        ocam = new Camera(resolution_, frame_rate_, port_);
