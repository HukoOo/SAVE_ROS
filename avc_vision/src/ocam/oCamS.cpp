...
public:
    ...
    bool getImages(cv::Mat &image, uint32_t &time_stamp) {
        ...
        if (camera->get_frame(srcImg.data, camFormat.image_size, 3) != -1)
        {
            // time stamp
...
private:
    ...
    void device_poll() {
        ...
        info_manager.loadCameraInfo( "package://avc_vision/config/left.yaml");
        ...
        ocams->getImages(left_image, time_stamp);
...
/*
        while (ros::ok())
        {
            ...
            if (!ocams->getImages(left_image, time_stamp)) {
                usleep(1000);
                continue;
            } else {
